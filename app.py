...
def make_domain_tools(api_key: str, model_str: str):
    ...
    def knowledge_lookup(query: str) -> str:
        ...
        try:
            from langchain_openai import ChatOpenAI
            _extra = {"extra_body": {"expert_type": "auto"}} if "usdcode" in model_str else {}
            m = ChatOpenAI(
...
def build_agent(api_key: str, model_str: str, system_prompt: str, subagents_cfg: list):
    ...
    usdcode model needs extra_body={"expert_type":"auto"} passed via model_kwargs.
    Returns a LangGraph CompiledStateGraph.
    """
    # Clear any leftover Groq env vars that could confuse deepagents internals
    ...
    # usdcode model requires expert_type routing header
    is_usdcode = "usdcode" in model_str
    extra = {"extra_body": {"expert_type": "auto"}} if is_usdcode else {}
