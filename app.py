...
def make_domain_tools(api_key: str, model_str: str):
    ...
    def knowledge_lookup(query: str) -> str:
        ...
        try:
            from langchain_groq import ChatGroq
            m = ChatGroq(api_key=api_key, model=model_str, temperature=0.1, max_tokens=700)
    ...
    def summarize_text(text: str, style: str = "bullets") -> str:
        ...
        try:
            from langchain_groq import ChatGroq
            m = ChatGroq(api_key=api_key, model=model_str, temperature=0.05, max_tokens=600)
    ...
    def compare_options(option_a: str, option_b: str, criteria: str) -> str:
        ...
        try:
            from langchain_groq import ChatGroq
            m = ChatGroq(api_key=api_key, model=model_str, temperature=0.2, max_tokens=900)
            r = m.invoke([HumanMessage(content=(
    ...
    def extract_action_items(text: str) -> str:
        ...
        try:
            from langchain_groq import ChatGroq
            m = ChatGroq(api_key=api_key, model=model_str, temperature=0.05, max_tokens=500)
            r = m.invoke([HumanMessage(content=(
