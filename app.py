...
def make_domain_tools(api_key: str, model_str: str):
    ...
    def knowledge_lookup(query: str) -> str:
        ...
        try:
            from langchain_groq import ChatGroq
            m = ChatGroq(api_key=api_key, model=model_str, temperature=0.1, max_tokens=400)
    ...
    def summarize_text(text: str, style: str = "bullets") -> str:
        ...
        try:
            from langchain_groq import ChatGroq
            m = ChatGroq(api_key=api_key, model=model_str, temperature=0.05, max_tokens=350)
    ...
    def compare_options(option_a: str, option_b: str, criteria: str) -> str:
        ...
        try:
            from langchain_groq import ChatGroq
            m = ChatGroq(api_key=api_key, model=model_str, temperature=0.2, max_tokens=450)
            r = m.invoke([HumanMessage(content=(
    ...
    def extract_action_items(text: str) -> str:
        ...
        try:
            from langchain_groq import ChatGroq
            m = ChatGroq(api_key=api_key, model=model_str, temperature=0.05, max_tokens=300)
            r = m.invoke([HumanMessage(content=(
...
def build_agent(api_key: str, model_str: str, system_prompt: str, subagents_cfg: list):
    ...
    # Use ChatGroq directly — most reliable with Groq
    from langchain_groq import ChatGroq
    main_model = ChatGroq(
        api_key=api_key,
        model=model_str,
...
with st.sidebar:
            ...
            if st.session_state.groq_key:
                with st.spinner("…"):
                    try:
                        ...
                        from langchain_groq import ChatGroq as _GroqVerify
                        m = _GroqVerify(api_key=st.session_state.groq_key,
    ...
    MODEL = st.selectbox(
        ...
        index=0,   # 8b-instant is default — fits in free tier 12k TPM
        key="model_selector",
    )
    model_notes = {
        "llama-3.1-8b-instant":    "✅ Recommended — fits free tier (12k TPM)",
        "llama-3.3-70b-versatile": "⚠️ May hit 12k TPM limit on free tier",
        "llama-3.1-70b-versatile": "⚠️ May hit 12k TPM limit on free tier",
        "mixtral-8x7b-32768":      "✅ Good — 32k context window",
        "gemma2-9b-it":            "✅ Lightweight — fast",
    }
    st.caption(model_notes.get(MODEL, ""))
    if "70b" in MODEL:
        st.warning("⚠️ 70b models use ~13k tokens with multi-agent prompts — may exceed Groq free tier (12k TPM). Use **llama-3.1-8b-instant** to avoid errors.", icon="⚠️")
...
<div class="res-wrap">
  ...
  <div class="res-body">{_html.escape(final_output)}</div>
                ...
                if "413" in err_str or "Request too large" in err_str or "tokens per minute" in err_str.lower():
                    ...
                    st.warning(
                        "**Fix:** Switch to **llama-3.1-8b-instant** in the sidebar — "
                        "it uses far fewer tokens and fits within Groq's free tier (12k TPM).\n\n"
                        "The 70b models send ~13k tokens with multi-agent prompts, exceeding the 12k limit."
                    )
                else:
...
<div class="res-wrap">
  ...
  <div class="res-body">{_html.escape(final_output)}</div>
            ...
            err_str = str(e)
            if "413" in err_str or "Request too large" in err_str or "tokens per minute" in err_str.lower():
                st.error("🔴 **Token limit exceeded (Error 413)** — switch to **llama-3.1-8b-instant** in the sidebar.")
            else:
                st.error(f"❌ {e}")
            with st.expander("Traceback"):
                st.code(traceback.format_exc())
